#!/usr/bin/env bash
# Megatron + V1 async training for the blackbox hermes-agent recipe.
#
...
CONFIG_NAME="${CONFIG_NAME:-hermes_agent_megatron_v1}"
...
# ── Agent parameters ─────────────────────────────────────────────────────
# AGENT_MAX_TURNS is the agent's turn budget inside the sandbox;
# read by the entrypoint script via the AGENT_MAX_TURNS env var.
RUNNER="${RUNNER:-hermes_agent}"
AGENT_MAX_TURNS="${AGENT_MAX_TURNS:-100}"
if [[ "${RUNNER}" == "hermes_agent" ]]; then
    AGENT_RUNNER_FQN="examples.blackbox_recipes.hermes_agent.hermes_agent_runner.hermes_agent_runner"
    HERMES_AGENT_TOOL_IMAGE="${HERMES_AGENT_TOOL_IMAGE:-swr.cn-east-3.myhuaweicloud.com/openyuanrong/hermes-agent-tool:latest}"
else
    echo "Unknown RUNNER=${RUNNER}; this recipe currently supports hermes_agent only" >&2
    exit 1
fi
...
RUNNER_ARGS=(
    ...
    "actor_rollout_ref.rollout.custom.agent_framework.agent_runners.hermes_agent.runner_fqn=${AGENT_RUNNER_FQN}"
    "actor_rollout_ref.rollout.custom.agent_framework.agent_runners.hermes_agent.dispatch_mode=ray_task"
    "actor_rollout_ref.rollout.custom.agent_framework.agent_runners.hermes_agent.max_concurrent_sessions=${MAX_CONCURRENT_SESSIONS}"
    "actor_rollout_ref.rollout.custom.agent_framework.agent_runners.hermes_agent.runner_kwargs.tool_image=${HERMES_AGENT_TOOL_IMAGE}"
    "actor_rollout_ref.rollout.custom.agent_framework.agent_runners.hermes_agent.runner_kwargs.run_timeout=${SWE_AGENT_RUN_TIMEOUT}"
    "actor_rollout_ref.rollout.custom.agent_framework.agent_runners.hermes_agent.runner_kwargs.conda_env=${CONDA_ENV}"
)
...
PROJECT_NAME="${PROJECT_NAME:-hermes_agent}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-hermes_agent_$(date +%Y%m%d_%H%M)}"
...
export HERMES_AGENT_TOOL_IMAGE
...
echo "=== Hermes Agent Blackbox Megatron Async Training ==="
echo "Model:       ${MODEL_PATH}"
echo "Train data:  ${TRAIN_DATA}"
echo "Val data:    ${VAL_DATA}"
echo "Engine:      ${ENGINE} (gen_tp=${GEN_TP}, train_tp=${TRAIN_TP})"
echo "Runner:      ${RUNNER}"
echo "Tool image:  ${HERMES_AGENT_TOOL_IMAGE}"
...
MAIN_CMD=(
    ...
    --config-path="${REPO_ROOT}/examples/blackbox_recipes/hermes_agent/config" \
