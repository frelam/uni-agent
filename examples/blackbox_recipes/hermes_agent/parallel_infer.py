"""Standalone inference runner for the blackbox hermes-agent recipe.
...
(config/hermes_agent_megatron_v1.yaml); its megatron/optimizer sections are
inert here since this driver never builds the actor worker group.

Usage:
    python examples/blackbox_recipes/hermes_agent/parallel_infer.py \
...
# ── Recipe-specific constants (only these differ between recipes) ───────────
_CONFIG_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "config")
_CONFIG_NAME = "hermes_agent_megatron_v1"
_DEFAULT_TOOL_IMAGE = "swr.cn-east-3.myhuaweicloud.com/openyuanrong/hermes-agent-tool:latest"
...
def _build_prompts(samples: list[dict[str, Any]]) -> tuple[Any, list[str]]:
    ...
    tools_kwargs_list = [
        dict((sample.get("extra_info") or {}).get("tools_kwargs", {}))
...
def _report(samples, uids, captured_scores) -> dict[str, Any]:
    ...
    per_sample_scores = [
        per_sample_sum[i] / per_sample_cnt[i] if per_sample_cnt[i] else 0.0
    ...
    *,
    ...
    try:
        ...
        if not captured_scores:
            logger.warning(
                "No trajectory scores captured — all rollouts may have failed; "
                "resolve rate will be reported as 0."
            )
...
def main():
    parser = argparse.ArgumentParser(description="Blackbox hermes-agent standalone inference")
