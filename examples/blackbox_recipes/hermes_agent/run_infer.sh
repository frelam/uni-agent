#!/usr/bin/env bash
# Standalone inference for the blackbox hermes-agent recipe.
# Runs rollout + reward only (no Megatron trainer) and reports resolve rate.
#
# Usage:
#   bash examples/blackbox_recipes/hermes_agent/run_infer.sh
#
...
echo "=== Hermes Agent Blackbox Inference ==="
...
python examples/blackbox_recipes/hermes_agent/parallel_infer.py \
    ...
    --tool-image "${HERMES_AGENT_TOOL_IMAGE}" \
