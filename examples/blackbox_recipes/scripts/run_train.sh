...
if [[ "${RUNNER}" == "mini_swe" ]]; then
    ...
    CONFIG_PATH="${CONFIG_PATH:-${REPO_ROOT}/examples/blackbox_recipes/mini_swe_agent/config}"
elif [[ "${RUNNER}" == "claude_code" ]]; then
    AGENT_RUNNER_FQN="examples.blackbox_recipes.claude_code.claude_code_runner.claude_code_runner"
    SWE_AGENT_TOOL_IMAGE="${SWE_AGENT_TOOL_IMAGE:-swr.cn-east-3.myhuaweicloud.com/openyuanrong/claude-code-tool:latest}"
    CONFIG_NAME="${CONFIG_NAME:-claude_code_megatron_v1}"
    CONFIG_PATH="${CONFIG_PATH:-${REPO_ROOT}/examples/blackbox_recipes/claude_code/config}"
else
    echo "Unknown RUNNER=${RUNNER}; expected mini_swe or claude_code" >&2
    exit 1
fi
